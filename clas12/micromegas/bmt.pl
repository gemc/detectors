...
my $bmt_z               = $parameters{"FMT_mothervol_zmin"} - $bmt_dz;      
                        # = 298.3-385 = -86.7 mm, MV center relative to CLAS center
my $bmt_zpcb            = $parameters{"BMT_endPCB_zpos"} - $bmt_z;          
                        # = 290.3+86.7 = 377.0 mm, end pcb in MV frame
...
sub place_forwardinterface
{
	   ...
	   # attachments for stiffeners:                
		...
		{
		my $layer_no= $l + 1;
		if($layer_no == $nlayer) {$Px = 7.9/2.0; $Pz = 7.0/2.0; $z = $bmt_zpcb - $Pz; }
