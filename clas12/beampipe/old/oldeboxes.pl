...
sub build_crate
{
	for(my $i=1; $i<=$NUM_BOXES; $i++)
	{
		...
		my $xpos = $x - 116.89*cos(rad($theta));
		my $ypos = $y - 116.89*sin(rad($theta));
		$detector{"name"}        = "Nearside_$pnumber";
		...
		my $zpos = $z - 113.27;
		$detector{"name"}        = "Back_$pnumber";
		$detector{"mother"}      = "ebox";
		$detector{"description"} = "Back of Crate";
		$detector{"pos"}         = "$x*mm $y*mm $zpos*mm";
		$detector{"rotation"}    = "0*deg 0*deg -$theta*deg";
		$detector{"color"}       = "aaaaaa";
		$detector{"type"}        = "Box";
		...
		$xpos = $x + 118.14*cos(rad($theta));
		$ypos = $y + 118.14*sin(rad($theta));
		$detector{"name"}        = "Farside_$pnumber";
		$detector{"mother"}      = "ebox";
		...
		$xpos = $x - 132.25*sin(rad($theta));
		$ypos = $y + 132.25*cos(rad($theta));
		$detector{"name"}        = "top_$pnumber";
		$detector{"mother"}      = "ebox";
		...
		$xpos = $x + 174.9*sin(rad($theta));
		$ypos = $y - 174.9*cos(rad($theta));
		$zpos = $z + 74.6;
...
sub build_panel
{
	for(my $i=1; $i<=$NUM_BOXES; $i++)
	{
		...
		for(my $n=1; $n<=$NUM_FEUS; $n++)
		{
			my $q = $i*100;
			$q = $q + $n;
			my $xpo = $x + (($n-1)*25.42-99.18)*cos(rad($theta));
			my $ypo = $y + (($n-1)*25.42-99.18)*sin(rad($theta));
			my $zpo = 112.62;
			my %detector = init_det();
			$detector{"name"}        = "panel_$q";
			$detector{"mother"}      = "ebox";
			$detector{"description"} = "Panel $q";
			$detector{"pos"}         = "$xpo*mm $ypo*mm $zpo*mm";
			$detector{"rotation"}    = "0*deg 0*deg -$theta*deg";
			...
			$xpo = $xpo - 10.5*cos(rad($theta));
			$ypo = $ypo - 10.5*sin(rad($theta));
			$zpo = $zpo - 113;
			$detector{"name"}        = "Lplate_$q";
			$detector{"mother"}      = "ebox";
			$detector{"description"} = "Left plate $q";
			$detector{"pos"}         = "$xpo*mm $ypo*mm $zpo*mm"; # 5*cm
			$detector{"rotation"}    = "0*deg 0*deg -$theta*deg";
			...
			$xpo = $xpo + 21*cos(rad($theta));
			$ypo = $ypo + 21*sin(rad($theta));
			...
			$xpo = $xpo - 6.5*cos(rad($theta));
			$ypo = $ypo - 6.5*sin(rad($theta));
