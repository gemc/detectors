...
void init_parameters()
{
	...
	for(int s=0; s<NSEG; s++)
  {
    in >> dummy >> wc0[0][s] >> wc0[1][s] >> pmt0[0][s] >> pmt0[1][s] >> wcr[s] >> pmtr[s] >> dplwc[s] >> wcang[s];
		wc0[2][s] = 0;
    pmt0[2][s] = 0;
  }
	...
	for(int s=0; s<NSEG; s++)
	{
		in >> hwde[s] >> hwdh[s] >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy >> dummy ;
  }
  ...
  // filling c4f10 refraction index using Sellmeier equaion
	for(int i=0; i<NP; i++)
	{
		...
		//		c4f10n[i]  = sqrt(1.28604141 + (1.07044083*l*l)/(l*l-0.0100585997) + (1.10202242*l*l)/(l*l-100));
...
void write_parameters()
{
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<5; p++)
		{
			...
			OUT.width(14) ;
			OUT << pe0[p][s] << "\t | cm | ellipse segment " << s+1 << " parameter p" ;
			OUT.width(4) ;
			OUT << p << "\t | " ;
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<5; p++)
		{
			...
			// par value, units, comment
			OUT.width(14) ;
			OUT << ph0[p][s] << "\t | cm | hyperbola segment " << s+1 << " parameter p" ;
			OUT.width(4) ;
			OUT << p << "\t | " ;
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<2; p++)
		{
			...
			// par value, units, comment
			OUT.width(14) ;
			OUT << x12y12[p][s] << "\t | cm | ellipse span point " << s+1 << " parameter " << spoints[p] << "\t | " ;
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<2; p++)
		{
			...
			// par value, units, comment
			OUT.width(14) ;
			OUT << x21y21[p][s] << "\t | cm | hyperbola edge points  " << s+1 << " parameter " << epoints[p] << "\t | " ;
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<2; p++)
		{
			...
			// par value, units, comment
			OUT.width(14) ;
			OUT << x11y11[p][s] << "\t | cm | PMT, ell. edges " << s+1 << " parameter " << ppoints[p] << "\t | " ;
	...
	for(int s=0; s<NSEG; s++)
	{
		...
		// par value, units, comment
		OUT.width(14) ;
		OUT << theta[s] << "\t | degrees | ell. theta at midline of mirror " << s+1 << " \t | " ;
	...
	for(int s=0; s<NSEG; s++)
		for(int p=0; p<2; p++)
		{
			...
			// par value, units, comment
			OUT.width(14) ;
			OUT << pmt0[p][s] << "\t | cm | PMT center for " << s+1 << ", coordinate  " << pmptcenter[p] << "\t | " ;
	...
	OUT << "nmirrors | \t 18 |  na | number of mirrors on one side |  vlassov, ungaro | vlassov@jlab.org, ungaro@jlab.org | ";
