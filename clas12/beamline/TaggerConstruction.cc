...
branch   1.1.1;
access   ;
symbols  first:1.1.1.1 megh:1.1.1;
locks    ; strict;
comment  @// @;


1.1
date     2009.05.05.22.11.47;  author niroula;  state Exp;
branches 1.1.1.1;
next     ;
...
#include "TaggerConstruction.hh"
#include "CLAS_Materials.hh"
...
//1/4"thick Lead to cover tagger Hodo
 ...
 if(OPTION == 3)
 {
  Tagger.Detectors.push_back(tag_modendplate_box);    // Modified endplate
  Tagger.Detectors.push_back(tag_Al_frame);           // connector plate frame between endplate and helium bag
  Tagger.Detectors.push_back(tag_Al_window);          // connector window     
  Tagger.Detectors.push_back(tag_heliumbag);          
 }
 ...
 // %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 // Putting everything into mother
 // %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

 Tagger.Place_Detectors(mother);

}

TaggerConstruction::~TaggerConstruction() {;}
...
1.1.1.1
log
@tpe_simulation added by megh on 4-30-09
@
text
@@
