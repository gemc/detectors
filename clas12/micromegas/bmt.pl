...
#     upstrm will have to be introduced and ctr revisited when introducing upstream endplate 002 -> temporary
{
	...
	my $Px        =  93.0/2.0; # 234-141=93, instead of 92.5, but ignoring holders and screws
	my $Py        =   2.0/2.0;
	...
	for(my $s = 0; $s < $ntile; $s++)
	{
		...
		my $x         = (141.0 + $Px)*cos($theta_rot*$pi/180.0);
		my $y         = (141.0 + $Px)*sin($theta_rot*$pi/180.0);
	...
	for(my $s = 0; $s < $ntile; $s++)
	{
		my   $element_no= $s + 1;
		my    $theta_rot = 30.0 + $s*120.0;
		my $x         = (144.0 + $Px)*cos($theta_rot*$pi/180.0);
		my $y         = (144.0 + $Px)*sin($theta_rot*$pi/180.0);
...
sub place_arcs
# check if interference when introducing upstream endplate: temporary?
{
	my $vname      = "BMT_Arcs";
...
sub place_forwardinterface
{
	...
	my $PSPhi     = 0.;
	my $PDPhi     = 360.;
	my @rmin      = (140.0, 141.0, 144.0, 215.0);
	my @rmax      = (144.0, 144.0, 148.0, 234.0);
	my @dz        = ( 15.0,  10.0,   8.0,   8.0);
	my @z         = (  0.5, -12.0,   4.0,   4.0);
	for(my $r = 0; $r < 4; $r++)
	{
		my $ring_no= $r + 1;
		my $PRMin  = $rmin[$r];
	...
	# attachments for stiffeners (oversimplified, hopefully temporary):

	my $Py        =  20.0/2.0;                # temporary
	my $Px        =  (214.76 - 148.0)/2.0;
	# 214.76 <= sqrt (215**2 - $Py**2) in order for Box not to overlap with 3rd ring above;
	my $Pz        = 15.0/2.0;                 # temporary
	my $z         = $bmt_zpcb + 8.0 - $Pz;    # temporary
	...
	for(my $s = 0; $s < $ntile; $s++)
	{
		...
		my %detector = init_det();
		$detector{"name"}        = "$vname\_attachment$element_no";
		$detector{"mother"}      = $envelope;
		$detector{"description"} = "$descriptio";
		$detector{"pos"}         = "$x*mm $y*mm $z*mm";
		$detector{"rotation"}    = "0*deg 0*deg -$theta_rot*deg";
