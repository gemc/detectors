...
#my $ColdHubIR     =  2.38*$inches ;     # total IR, OR as from the drawings in M:\hallb_eng\CLAS12\Magnets\Torus\Torus April 08 parts
#my $ColdHubLength =  2022.4/2.0;   # taken from new drawing sent by L. Quettier (April 2010)
my $ColdHubIR     =  175./2. ;     # taken from new drawing sent by L. Quettier (April 2010)
my $ColdHubOR     =  4.72*$inches ;


# Air coils:
my $TorusAirLength = 94.*$inches/2.0;  # 1/2 length 


# Steel frame:
my $SteelFrameLength = $TorusAirLength;
...
my $ipipe_le = 93.5*$inches/2.0;     # choosen to match SteelFrameLength defined above (should be 2278 mm = 89.6" according to LQ drowings)
...
my $ishield_le = 2201.8/2.0;	# taken from new drawing sent by L. Quettier (April 2010)


# Front and back Plates:
my $face_plate_IR = 127./2. + .1;         # chosen to match Torus Inner Pipe defined above
#my $face_plate_IR = 60./2. ;               # modified (R. De Vita October 2010) to match A. Vlassov Inner Pipe Design
my $face_plate_OR = 7.64*$inches - .1;
my $face_plate_LE = 1.0*$inches/2.0;                      # based on information from D. Kashy (April 2010)
my $fp_zpos       = $TorusZpos - $SteelFrameLength + $face_plate_LE - 0.1; # Allow microgap to avoid overlaps
my $bp_zpos       = $TorusZpos + $SteelFrameLength - $face_plate_LE + 0.1;
...
{
        my %detector = init_det();
	$detector{"name"}        = "torus_center_tube_shield";
	$detector{"mother"}      = "fc";
	...
	$detector{"type"}        = "Tube";
	$detector{"dimensions"}  = "$ishield_ir*mm $ishield_or*mm $ishield_le*mm 0*deg 360*deg";
...
sub make_w_shields
{
	for(my $n=0; $n<6; $n++)
	{
		...
		my $dy     = 232.0/2.0;          # length, cm
		my $dz     = 2.0/2.0; # heigth, cm


		my $R      = 1.05*$inches/2.0;
